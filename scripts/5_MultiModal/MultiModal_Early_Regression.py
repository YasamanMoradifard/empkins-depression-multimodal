...
def build_inner_pipeline_early_fusion(reg, rfe_n: int) -> Pipeline:
    """Early fusion inner: RFE(n) on concatenated features -> regressor. Correlation per modality done separately.
    RFE uses DecisionTreeRegressor for consistent, fast feature selection across all final regressors."""
...
def evaluate_model(model, X_test: pd.DataFrame, y_test: np.ndarray) -> Dict[str, float]:
    ...
    try:
        ...
        print(f"[WARNING] Model prediction failed in evaluate_model: {e}")
        # Return mean prediction as fallback
        y_pred = np.full(len(y_test), np.nanmean(y_test))
...
def plot_rmse_comparison(
    ...
    """Bar chart comparing RMSE across individual modalities and fused (early fusion) model."""
    ...
    plt.ylabel("RMSE")
    plt.title(f"RMSE Comparison — {model_name} (Early Fusion)")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def parse_args():
    ...
    p.add_argument(
        "--phq9_data_path",
        type=str,
        default=str(PHQ9_SCORE_DATA_PATH),
        help="Path to Data_D02_info.csv containing ID and PHQ9-Score columns",
    )
    p.add_argument(
        "--aggregation_method",
        type=str,
    ...
    p.add_argument("--regressors", nargs="*", default=None,
                   help="Regressor names to run (default: all). E.g. LinearRegression RandomForest") # LogisticRegression, KNN, RandomForest, DecisionTree, XGBoost, AdaBoost
    ...
    p.add_argument("--output_dir", type=str, default="/home/vault/empkins/tpD/D02/Students/Yasaman/5_MultiModal_ML/Early Fusion Regression Results",
                   help="Base output directory. Results saved in: <output_dir>/Early_Fusion_{mod}_{cond}_{phases}_{agg}/ (default: script dir)")
    args = p.parse_args()
    return args
...
def main(cfg) -> None:
    ...
    phq9_path = Path(getattr(cfg, "phq9_data_path", str(PHQ9_SCORE_DATA_PATH)))
    print("Dashboard:")
    ...
    print(f"Output base directory: {cfg.output_dir or '(script dir)'}")
    print("-----------------------------------------\n")
    ...
    for mod in modalities:
        if mod == "text":
            df = load_text_data(text_path, condition=condition, phq9_path=phq9_path)
            
        elif mod == "audio":
            df = load_audio_data(condition, phases, opensmile_dir, aggregation_method=aggregation_method, phq9_path=phq9_path)
            
        elif mod == "video":
            df = load_video_data(condition, phases, opendbm_data_dir=opendbm_dir, aggregation_method=aggregation_method, phq9_path=phq9_path)
    ...
    for mod in modalities:
        ...
        d["X_aligned"] = X_mod
        d["y_aligned"] = df_mod["label"].to_numpy(dtype=float)
    ...
    # ============================
    # MODEL SETUP (REGRESSORS + PARAM GRIDS)
    # ============================
    print("-----------------------------------------\n")
    print("Model setup")
    print("-----------------------------------------\n")
    ...
    if cfg.regressors:
        models = {k: v for k, v in models.items() if k in cfg.regressors}
    if not models:
        raise ValueError("No valid regressors selected")
    param_grids_reg = get_param_grids_reg_only()

    # Create one subfolder per regressor (e.g. KNN, RandomForest, LinearRegression)
    ...
    pred_accum: Dict[str, Tuple[List[float], List[float]]] = {}
    best_pipes: Dict[str, Pipeline] = {}  # last fold's best pipeline per model for feature importance
    last_feature_names: List[str] = []   # from last fold
    unimodal_oof_rmse_by_model: Dict[str, List[Dict[str, float]]] = {}  # per model: list of {mod: rmse} per fold
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf_outer.split(y, y, groups=groups), start=1):
        ...
        for mod in modalities:
            ...
            if mod == "text":
                ...
                corr_sel = CorrelationSelector(n_features_to_select=min(mw_per_mod, X_tr.shape[1]))
                corr_sel.fit(X_tr, y_train)
                names = corr_sel.get_feature_names_out(list(X_tr.columns))
                selected_per_mod[mod] = names
        ...
        last_feature_names = feature_names_cat
        ...
        for model_name, reg in models.items():
            ...
            best_inner_score = -np.inf  # for R², higher is better
            best_pipe = None
            grid_reg = param_grids_reg.get(model_name, {})

            # Fix for KNN: filter out n_neighbors values that exceed available training samples
            if model_name == "KNN" and "reg__n_neighbors" in grid_reg:
                min_train_samples = float('inf')
                for tr_idx, _ in cv_inner.split(X_train_cat, y_train, groups=groups_train):
                    min_train_samples = min(min_train_samples, len(tr_idx))
                original_n = grid_reg["reg__n_neighbors"]
                filtered_n = [n for n in original_n if n <= min_train_samples]
                if not filtered_n:
                    filtered_n = [min(1, int(min_train_samples))]
                grid_reg = grid_reg.copy()
                grid_reg["reg__n_neighbors"] = filtered_n
            ...
            for n_rfe in rfe_n_values:
                inner_pipe = build_inner_pipeline_early_fusion(clone(reg), rfe_n=n_rfe)

                search = GridSearchCV(
                    inner_pipe, grid_reg, cv=cv_inner, scoring="r2", refit=True,
                    n_jobs=1, error_score="raise",
                )
                try:
                    if is_xgb:
                        # Ensure data compatibility for XGBoost
                        X_train_cat_clean, y_train_clean = _ensure_xgboost_compatible(X_train_cat, y_train)
                    else:
                        X_train_cat_clean = X_train_cat
                        y_train_clean = y_train
                    
                    search.fit(X_train_cat_clean, y_train_clean, groups=groups_train)
                    if search.best_score_ > best_inner_score:
                        best_inner_score = search.best_score_
                        best_pipe = search.best_estimator_
                except Exception as e:
                    print(f"[WARNING] GridSearchCV failed for {model_name} with RFE n={n_rfe}: {e}")
                    continue

            if best_pipe is None:
                best_pipe = build_inner_pipeline_early_fusion(clone(reg), rfe_n=1)
                try:
                    if is_xgb:
                        X_train_cat_clean, y_train_clean = _ensure_xgboost_compatible(X_train_cat, y_train)
                    else:
                        X_train_cat_clean = X_train_cat
                        y_train_clean = y_train
                    best_pipe.fit(X_train_cat_clean, y_train_clean)
                except Exception as e:
                    print(f"[ERROR] Failed to fit fallback pipeline for {model_name}: {e}")
            ...
            # Per-modality OOF RMSE for rmse_comparison plot (same rationale as Late Fusion)
            unimodal_oof_rmse_this_fold: Dict[str, float] = {}
            cv_inner_rmse = GroupKFold(n_splits=inner_folds)
            for mod in modalities:
                X_tr_mod = X_train_per_mod[mod][selected_per_mod[mod]].copy()
                n_feat = X_tr_mod.shape[1]
                if n_feat < 1:
                    unimodal_oof_rmse_this_fold[mod] = float("inf")
                    continue
                rfe_n = min(10, n_feat)
                pipe_mod = Pipeline([
                    ("scaler", StandardScaler()),
                    ("rfe", RFE(estimator=DecisionTreeRegressor(max_depth=5, random_state=RANDOM_STATE), n_features_to_select=rfe_n, step=1)),
                    ("reg", clone(reg)),
                ])
                try:
                    if is_xgb:
                        X_tr_mod_clean, y_train_clean = _ensure_xgboost_compatible(X_tr_mod, y_train)
                    else:
                        X_tr_mod_clean = X_tr_mod
                        y_train_clean = y_train
                    oof_pred = cross_val_predict(pipe_mod, X_tr_mod_clean, y_train_clean, cv=cv_inner_rmse, groups=groups_train)
                    rmse = float(np.sqrt(mean_squared_error(y_train_clean, oof_pred)))
                except Exception as e:
                    print(f"[WARNING] cross_val_predict failed for {model_name} modality {mod}: {e}")
                    rmse = float("inf")
                unimodal_oof_rmse_this_fold[mod] = rmse
            if model_name not in unimodal_oof_rmse_by_model:
                unimodal_oof_rmse_by_model[model_name] = []
            unimodal_oof_rmse_by_model[model_name].append(unimodal_oof_rmse_this_fold)
            ...
            try:
                if is_xgb:
                    X_test_cat_clean = _ensure_xgboost_compatible(X_test_cat)
                else:
                    X_test_cat_clean = X_test_cat
                test_metrics = evaluate_model(best_pipe, X_test_cat_clean, y_test)
            except Exception as e:
                print(f"[ERROR] Failed to evaluate {model_name} on test set: {e}")
                # Use default metrics
                test_metrics = {
                    "MAE": float("inf"),
                    "RMSE": float("inf"),
                    ...
                    "y_pred": [np.nanmean(y_test)] * len(y_test),
                }
            all_results.append({
            ...
            if model_name not in pred_accum:
                pred_accum[model_name] = (test_metrics["y_true"], test_metrics["y_pred"])
            else:
                pred_accum[model_name] = (
                    pred_accum[model_name][0] + test_metrics["y_true"],
                    pred_accum[model_name][1] + test_metrics["y_pred"],
                )

            print(f"    [R²]: {test_metrics['R2']:.3f}, [MAE]: {test_metrics['MAE']:.3f}, [Num_selected features]: {len(sel_names)}")
    ...
    summary = results_df.groupby("model").agg(
        ...
        Adjusted_R2_mean=("Adjusted_R2", "mean"), Adjusted_R2_std=("Adjusted_R2", "std"),
    ).reset_index()
    ...
    # Write per-model outputs: summary, results, regression plots
    for model_name in models:
        model_dir = out_dir / model_name
        ...
        if not model_summary.empty:
            ...
            final_row = pd.DataFrame([{
                "model": model_name,
                "MAE_avg": float(ms["MAE_mean"]), "MAE_std": float(ms["MAE_std"]),
                "RMSE_avg": float(ms["RMSE_mean"]), "RMSE_std": float(ms["RMSE_std"]),
                "MAPE_avg": float(ms["MAPE_mean"]), "MAPE_std": float(ms["MAPE_std"]),
                "R2_avg": float(ms["R2_mean"]), "R2_std": float(ms["R2_std"]),
                "Adjusted_R2_avg": float(ms["Adjusted_R2_mean"]),
                "Adjusted_R2_std": float(ms["Adjusted_R2_std"]),
            }])
        ...
        if model_name in pred_accum and pred_accum[model_name][0] and pred_accum[model_name][1]:
            y_true_plot = np.array(pred_accum[model_name][0])
            y_pred_plot = np.array(pred_accum[model_name][1])
            agg_metrics = {
        ...
        # Feature importance (using last fold's fitted pipeline; RFE-selected features)
        if model_name in best_pipes and last_feature_names:
            pipe = best_pipes[model_name]
            # RFE returns reduced features; get names from pipeline
            if "rfe_selector" in pipe.named_steps:
                sel_mask = pipe.named_steps["rfe_selector"].support_
                sel_names = [last_feature_names[i] for i in range(len(last_feature_names)) if sel_mask[i]]
            else:
                sel_names = last_feature_names
            plot_feature_importance(pipe, sel_names, model_name,
                                   save_path=model_dir / "feature_importance.png")
        # RMSE comparison (unimodal OOF vs fused test RMSE)
        if model_name in unimodal_oof_rmse_by_model and unimodal_oof_rmse_by_model[model_name] and not model_summary.empty:
            folds_data = unimodal_oof_rmse_by_model[model_name]
            unimodal_rmse_mean = {
                mod: float(np.mean([f[mod] for f in folds_data if mod in f and np.isfinite(f[mod])]))
                for mod in modalities
                if any(mod in f for f in folds_data)
            }
            fused_rmse = float(model_summary.iloc[0]["RMSE_mean"])
            if unimodal_rmse_mean and np.isfinite(fused_rmse):
                plot_rmse_comparison(
                    unimodal_rmse_mean,
                    fused_rmse,
                    model_name,
                    save_path=model_dir / "rmse_comparison.png",
                )
        print("-----------------------------------------\n")
    # Config at root
    cfg_dict = {k: getattr(cfg, k) for k in ["fusion_modalities", "condition", "phases", "minutes", "outer_folds", "inner_folds", "nan_threshold", "mannwhitney_per_modality", "aggregation_method", "text_data_path", "phq9_data_path"] if hasattr(cfg, k)}
    cfg_dict["target"] = "PHQ9-Score"
    cfg_dict["n_samples"] = int(len(y))
    ...
    print(f"[INFO] Results and selected features saved under {out_dir} (one subfolder per regressor: {', '.join(models)})")
    print("-----------------------------------------\n")
