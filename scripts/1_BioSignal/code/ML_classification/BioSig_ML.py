...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="BioSig (EMG/ECG/RSP) ML with nested CV for depression detection")
    ...
    parser.add_argument(
        ...
        default="EMG",
        choices=["EMG", "ECG", "RSP"],
        help="BioSig modality: EMG, ECG, or RSP",
    )
    parser.add_argument(
        "--minutes",
        type=int,
        default=3,
        help="Window size in minutes (choices: 1, 3, 5)",
    )
    parser.add_argument(
        "--condition",
        type=str,
...
def main() -> None:
    ...
    print("=" * 80)
    print("[INFO] BioSig ML — Nested CV: build_preprocess_pipeline + Mann-Whitney → build_inner_pipeline (RFE + HPO)")
    print(json.dumps(vars(cfg), indent=2))
    print("=" * 80)

    mannwhitney_k = getattr(cfg, "mannwhitney_prefilter", 50)

    # Resolve phases for load_biosig_data
    ...
    out_dir = root_results_dir / f"{cfg.data_type}_{cfg.condition}_{phase_tag}_{aggregation_method}"
    out_dir.mkdir(parents=True, exist_ok=True)
    base_name = f"biosig_ml_{cfg.data_type}_{cfg.condition}_{phase_tag}_{aggregation_method}_nested_cv"

    # Load and aggregate BioSig data (load_biosig_data uses BioSigAggregation inside)
    df = load_biosig_data(
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(X, y, groups=groups), start=1):
        ...
        for model_name, base_clf in models.items():
            ...
            row = {"data_type": cfg.data_type, "condition": cfg.condition, "phases": phase_tag, "aggregation_method": aggregation_method,
    ...
    if all_results_rows:
        ...
        metric_cols = [c for c in results_df.columns
                      if c not in ["data_type", "condition", "phases", "aggregation_method", "outer_fold", "model"]]
