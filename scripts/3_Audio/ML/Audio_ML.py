#!/usr/bin/env python3
"""
Audio/OpenSmile ML with nested cross-validation for depression detection (leakage-free).

Pipeline structure:
1. Load audio data via load_audio_data (pre-aggregated CSV: Audio_data_aggregated_by_phase or by_ID).
2. Nested CV:
...
No correlation filter. Uses build_preprocess_pipeline, build_inner_pipeline, extract_selected_features, load_audio_data.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audio ML with leak-proof pipeline architecture (OpenSmile data)")
    ...
    parser.add_argument(
        ...
        help="Directory containing Audio_data_aggregated_by_phase.csv / Audio_data_aggregated_by_ID.csv, or path to a CSV",
    )
...
def main() -> None:
    ...
    print("=" * 80)
    print("[INFO] Audio ML — Nested CV: build_preprocess_pipeline + Mann-Whitney → build_inner_pipeline (RFE + HPO)")
    ...
    # Resolve phases for load_audio_data
    ...
    root_results_dir = Path(__file__).parent / "Audio_result_nested_CV"
    out_dir = root_results_dir / f"{cfg.condition}_{phase_tag}_{aggregation_method}"
    out_dir.mkdir(parents=True, exist_ok=True)
    base_name = f"audio_ml_{cfg.condition}_{phase_tag}_{aggregation_method}_nested_cv"

    # Load audio data (pre-aggregated CSV via load_audio_data)
    df = load_audio_data(
        condition=cfg.condition,
        phases=phases,
        opensmile_data_dir=opensmile_dir,
