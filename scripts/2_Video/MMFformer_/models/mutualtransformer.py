...
class MutualTransformer(nn.Module):
    def __init__(self, a_d=145, v_d=128, d=256, f_d=512, l=6, transformer_dropout=0.1):
        """
        ...
        Args:
            ...
            transformer_dropout (float): NEW: Dropout rate for transformer layers (default: 0.1)
        """
        super().__init__()
        ...
        # ========== NEW: Use transformer_dropout parameter (Added for regularization) ==========
        self.a_encoder = nn.TransformerEncoder(
            encoder_layer=nn.TransformerEncoderLayer(
                d_model=144, nhead=4, dim_feedforward=d, 
                dropout=transformer_dropout,  # NEW: Use parameter instead of default
                batch_first=True
            ),
            num_layers=l,
        )
        ...
        self.v_encoder = nn.TransformerEncoder(
            encoder_layer=nn.TransformerEncoderLayer(
                d_model=v_d, nhead=4, dim_feedforward=d, 
                dropout=transformer_dropout,  # NEW: Use parameter instead of default
                batch_first=True
            ),
            num_layers=l,
