...
class GenerateVisualModel(nn.Module):
    ...
    def _build_visual_model(self, model_name='vit_base_patch16', drop_path_rate=0.1, global_pool=False,
        ...
        if not os.path.isabs(ckpt_path):
            # Resolve relative to the models directory
            ckpt_path = Path(__file__).parent.parent / "pretrained_models" / "visualmae_pretrained.pth"
        ...
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(
                f"VisualMAE pretrained model not found: {ckpt_path}\n"
                f"Please ensure the file exists or update the ckpt_path parameter."
            )
