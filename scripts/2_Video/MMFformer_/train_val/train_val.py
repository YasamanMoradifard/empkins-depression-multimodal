...
def train_epoch(
    ...
    with tqdm(
        ...
        for x, y, mask in pbar:
            ...
            # Calculate TP, FP, TN, FN for F1 score
    ...
    final_accuracy = correct_count / sample_count
    ...
    final_accuracy = min(1.0, max(0.0, final_accuracy))
    final_precision = min(1.0, max(0.0, final_precision))
    final_recall = min(1.0, max(0.0, final_recall))
    final_f1 = min(1.0, max(0.0, final_f1))
...
def val(
    ...
    # Ensure both classes (0 and 1) are always present for binary classification
    all_possible_classes = [0, 1]
    class_labels = sorted(list(set(true_labels)))
    class_counts = {label: true_labels.count(label) for label in all_possible_classes}
    class_tp = {label: 0 for label in all_possible_classes}
    class_fp = {label: 0 for label in all_possible_classes}
    class_fn = {label: 0 for label in all_possible_classes}
    class_tn = {label: 0 for label in all_possible_classes}

    # Calculate TP, FP, FN, TN for both classes
    for true, pred in zip(true_labels, predicted_labels):
        # For class 1 (positive class)
        if true == 1 and pred == 1:
            class_tp[1] += 1
            class_tn[0] += 1  # Correctly predicted as 0 (TN for class 0)
        elif true == 0 and pred == 1:
            class_fp[1] += 1  # False positive for class 1
            class_fn[0] += 1  # False negative for class 0
        elif true == 1 and pred == 0:
            class_fn[1] += 1  # False negative for class 1
            class_fp[0] += 1  # False positive for class 0
        elif true == 0 and pred == 0:
            class_tp[0] += 1  # True positive for class 0
            class_tn[1] += 1  # Correctly predicted as 1 (TN for class 1)
    ...
    # Class-specific accuracy: For binary classification, this is the recall (TP / (TP + FN))
    # which measures how well we identify samples of that class
    class_accuracy = {
        label: min(1.0, class_tp[label] / (class_tp[label] + class_fn[label])) 
        if (class_tp[label] + class_fn[label]) > 0 else 0.0
        for label in class_labels
    }

    # Weighted and unweighted metrics
    # Note: weighted_accuracy here is actually weighted recall (class-balanced accuracy)
    weighted_accuracy = min(1.0, sum(class_accuracy[label] * class_counts[label] for label in class_labels) / num_samples)
    weighted_precision = min(1.0, sum(class_precision[label] * class_counts[label] for label in class_labels) / num_samples)
    weighted_recall = min(1.0, sum(class_recall[label] * class_counts[label] for label in class_labels) / num_samples)
    weighted_f1 = min(1.0, sum(class_f1[label] * class_counts[label] for label in class_labels) / num_samples)

    unweighted_accuracy = min(1.0, sum(class_accuracy.values()) / len(class_labels))
    unweighted_precision = min(1.0, sum(class_precision.values()) / len(class_labels))
    unweighted_recall = min(1.0, sum(class_recall.values()) / len(class_labels))
    unweighted_f1 = min(1.0, sum(class_f1.values()) / len(class_labels))
